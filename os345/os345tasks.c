               ...
               char *argv[])              // task argument pointers
{
    ...
    for (tid = 0; tid < MAX_TASKS; tid++)
    {
        if (tcb[tid].name == 0)
        {
            ...
            memset(tcb[tid].children, 0, MAX_TASKS);
            ...
            enq(&readyQ, tid);
            // the parent task (curTask) is creating child task (tid)
            tcb[curTask].children[tcb[curTask].nchildren] = tid;
            tcb[curTask].nchildren++;

            // call copmute clock time function
            // reschedule
...
static void exitTask(int taskId)
{
    ...
    // if(s = tcb[taskId].event)
    // then deleteId(&s->blockedQ, taskId)
...
int sysKillTask(int taskId)
{
    ...
    // reasssign this task children (tcb[taskId].children) to it's parent
    // (tcb[tid].parent)
