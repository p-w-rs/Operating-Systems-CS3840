...
int enq(PQ *q, int taskId)
{
    // add task: taskId to the q with priority tcb[taskId].priority
    // return 0 on success, 1 on failure
}

int deq(PQ *q)
{
    // removes the highet priority task from the q
...
void printq(PQ *q)
{
    // loops through the q and prints all items
}

int deleteId(PQ *q, int taskId)
{
    // loop trhough the Q and delete node with taskId
}
