...
// declare these function in os345.h and impelment here:
// void compute_clocks() ....
...
static int scheduler()
{
    ...
    {
        // 1. find a task from the readyQ to schedule
        // 2. if the clock time varialbe is > 0  decrement by 1 and return
        // 3. else loop back to 1.

        // Somewhere in the above:
        // if all varaibles are 0 then recompute clock times and reschedule
    }
...
void powerDown(int code)
{
    ...
    // free ready queue
    // loop trhough rq  and  free all nodes
    // free(rq);
